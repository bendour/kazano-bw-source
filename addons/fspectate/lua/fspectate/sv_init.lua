...
local function startSpectating(ply, target)
    ...
    BaseWars:Notify(ply, "You are now spectating " .. targetText, NOTIFICATION_GENERIC, 5)
end

local function Spectate(ply, cmd, args)
    if not BaseWars:IsAdmin(ply, true) then BaseWars:Notify(ply, "No Access", NOTIFICATION_ERROR , 5) return end

        local target = findPlayer(args[1])
        if target == ply then BaseWars:Notify(ply, "Invalid target!", NOTIFICATION_ERROR , 5) return end
...
net.Receive("FSpectateTarget", function(_, ply)
    ...
    startSpectating(ply, entity)
    hook.Run("FSpectate.StartSpectating", ply, entity)
end)
...
local function endSpectate(ply, cmd, args)
    ...
    hook.Run("FSpectate.StopSpectating", ply)
end
