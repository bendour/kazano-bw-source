...
function rRadio.utils.GetOwner( entity )
    ...
    -- First try NWEntity (set by boombox spawn function)
    local owner = entity:GetNWEntity( "Owner" )
    if IsValid( owner ) then return owner end
    
    -- Then try CPPI (used by prop protection addons and F4 menu)
    if entity.CPPIGetOwner then
        local cppiOwner = entity:CPPIGetOwner()
        if IsValid( cppiOwner ) then return cppiOwner end
    end
    
    -- Try Ownable (used by some addons)
    if entity.Ownable and entity.Ownable.GetOwner then
        local ownableOwner = entity.Ownable:GetOwner()
        if IsValid( ownableOwner ) then return ownableOwner end
    end
...
function rRadio.utils.CanInteractWithBoombox( ply, boombox )
    ...
    -- Check CAMI permissions (for admins to use others' boomboxes)
