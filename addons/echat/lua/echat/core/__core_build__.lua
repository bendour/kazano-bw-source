...
function echat:Build()
	...
	bottom_panel:DockPadding(5,1,5,4) --just few pixels
	echat.bottom_panel = bottom_panel
	function bottom_panel:Paint(w,h)
		draw.RoundedBoxEx(8,0,0,w,h,clr.main.bg,false,false,true,true)
	end

	local text_entry_font = echat:AdaptiveFont("echat", 16, 500)
	local _, th = esclib.util:TextSize("|",text_entry_font)
	local button_tall = th+4 -- font
	...
	function send_button:Paint(w,h)
		esclib.draw:MaterialCentered(w*0.5,h*0.5,w*0.4,clr.main.text,echat:GetMaterial("send.png"))
	end
	...
	function settings_button:Paint(w,h)
		esclib.draw:MaterialCentered(w*0.5,h*0.5,w*0.4,clr.main.text,esclib.Materials["cog.png"])
	end
	...
	function pnl:Paint(w,h)
		if self.opened then
			...
			if self.Dragging then
				draw.RoundedBoxEx(8,5,0,w-10,panel_top_height,clr.main.accent,true,true,false,false)
			else
				draw.RoundedBoxEx(8,5,0,w-10,panel_top_height,clr.main.bg2,true,true,false,false)
			end
			draw.SimpleText(echat.config.chat_name, font, 15, panel_top_height*0.5-1, clr.main.text, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		end
	end
	...
	if echat.config.emojies then
		...
		function emoji_button:Paint(w,h)
			esclib.draw:MaterialCentered(w*0.5,h*0.5,w*0.4,clr.main.text,echat:GetMaterial("emoji.png"))
		end
