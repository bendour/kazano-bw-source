...
function P:SellBitcoins(bitminer)
	...
	if table.HasValue(bitminer.authorisedPlayers, self) or bitminer.remoteUser == self then
        ...
        local xp = math.floor(BaseWars:CalculatePlayerXP(self, BaseWars:CalculateXPFromMultiplier(bitminer.bitcoin * BM2CONFIG.BitcoinValue) * .01))
		self:AddXP(xp)
		bitminer.bitcoin = 0
...
net.Receive("BM2.Command.SellBitcoins", function(len, ply)
	...
	if e:CPPIGetOwner() != ply then
		BaseWars:Notify(ply, "#bitminer_notYours", NOTIFICATION_ERROR, 5)
		return
	end
...
hook.Add("BaseWars:Initialize", "BITMINER", function()
	BaseWars:AddTranslation("bitminer_notYours", "fr", "Ce bitminer n'est pas à vous")
	BaseWars:AddTranslation("bitminer_notYours", "en", "This bitminer is not yours")
end)