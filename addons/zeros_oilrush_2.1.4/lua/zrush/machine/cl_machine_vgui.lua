...
function zrush.Machine.ModuleInfo(parent,m_id,Sell)
	...
	AddField(BaseWars:FormatMoney(value),zclib.GetFont("zclib_font_mediumsmall"),zclib.colors["green01"],9,150,20, RIGHT,false,top_con)
	/////////
	...
	for m_id, m_dat in pairs(PurchaseableItems) do
		...
		itm.Paint = function(s, w, h)
			...
			if UserHasAllowedRank == false or UserHasAllowedJob == false then
				...
				draw.SimpleText(BaseWars:FormatMoney(m_dat.price), zclib.GetFont("zclib_font_mediumsmall"), w - 10 * zclib.wM, h - 5 * zclib.hM, zclib.colors["green01"], TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM)
			end
