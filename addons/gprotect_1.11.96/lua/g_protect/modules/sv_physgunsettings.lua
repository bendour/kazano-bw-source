...
gProtect.HandlePhysgunPermission = function(ply, ent)
    ...
    if cfg.enabled then
        ...
        if !cfg.PickupVehiclePermission[usergroup] and !cfg.PickupVehiclePermission[secondary_usergroup] and !cfg.PickupVehiclePermission["*"] then
        ...
        if !isstring(owner) and IsValid(owner) and owner:IsPlayer() or owner == "Disconnected" then
            ...
            if permGroup["*"] or permGroup[usergroup] or permGroup[secondary_usergroup] or ply == owner then return true end
        end
    end
end
