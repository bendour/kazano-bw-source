...
Reward.GiveReward = function(ply, amount, key)
	if amount > 1 and ulx.tempadduser then
		ulx.tempadduser( Entity(0), ply, key, amount, "user" )
	else
		ulx.adduser( Entity(0), ply, key )
	end
end
/*-------------------------------------------------------------------------*/
else-------------------------------------------------------------------------
...
Reward.GetKey = function(name)
	local key = ULib.ucl.groups[name] and name or nil
	return key
end

Reward.LangPhrase = "ULX_KeyInfo"
