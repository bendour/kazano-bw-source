...
Reward.Name = "AShop Weapon Temporary"
...
Reward.NetWrite = function(rewardVal)
	...
	for k, v in pairs(ashop.items) do
        if v.name == rewardVal then
        	if ashop.object_types[v.object_types].UniqueIdentifier == "TempWeapons" then akey = v.id break end
        end
    end
...
Reward.LangPhrase = "AShop_Weapon_Temporary_KeyInfo"
