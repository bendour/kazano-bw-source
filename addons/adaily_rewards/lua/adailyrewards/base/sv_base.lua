...
-- NEW: Fonction pour donner 1 niveau par tâche complétée (sans vérifier les 3 tâches)
function ADRewards.GiveDayRewardForTask(ply)
	if !ADRewards.SeasonNow then return end
	local numrewards = #ply.SeasonADR.Rewards
	local nextreward = numrewards+1
...
function ADRewards.GiveTaskVal(ply, taskname, amount)
	...
	if plyTask.ValNow >= plyTask.ValNeed then
		...
		-- NEW: LevelPerTask - chaque tâche complétée donne 1 niveau
		if ADRewards.Config.LevelPerTask then
			ADRewards.GiveDayRewardForTask(ply)
		elseif ADRewards.Config.TasksForClaim then
			ADRewards.GiveDayReward(ply)
		end
