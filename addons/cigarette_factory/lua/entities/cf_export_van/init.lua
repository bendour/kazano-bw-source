...
function ENT:Use( activator, caller )
	...
	local cigAmount = caller.cfCigsAmount
	local payOut = cigAmount*self.sellingPrice

	caller:AddMoney(payOut)
    local xp = math.floor(BaseWars:CalculatePlayerXP(caller, BaseWars:CalculateXPFromMultiplier(payOut) * .01))
	caller:AddXP(xp)
	SendNotification(caller, 0, 4, cf.SellText1..cigAmount..cf.SellText2..BaseWars:FormatMoney(payOut).."!")
	caller.cfCigsAmount = 0
	self:EmitSound(self.SellSound, 80, 100, 1)

	hook.Run("CigaretteFactory:PlayerSellCigarettes", caller, self, cigAmount, payOut)
