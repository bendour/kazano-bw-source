...
local function draw_bg(w,h,hovered,clr)
	draw.RoundedBox(8,0,0,w,h,hovered and clr.button.hover or clr.button.main)
end
		...
		combolist:AddTab(tabname_translated,function(tab_content)
			...
			for _,lang in ipairs(table.GetKeys(add:GetLanguages())) do
				...
				function lbutton:Paint(w,h)
					...
					-- draw.SimpleText( lang_code,font,w-15,h*0.5,clr.button.text,TEXT_ALIGN_RIGHT,TEXT_ALIGN_CENTER)
					...
					if not selected and addon_language == lang then
						esclib.draw:MaterialCentered(w-checkbox_size-15, h*0.5, checkbox_size*0.6, clr.button.accent, true_mat)
	...
	combolist:AddTab(tabname_translated,function(tab_content)
		...
		for name,skin in pairs(add.data.skins) do
			...
			function lbutton:Paint(w,h)
				...
				if not selected and addon_active_skin == name then
					draw.RoundedBox(0,0,h-3,w,3,clr.default.red)
				end
			end
			...
			context:AddButton(esclib.addon:Translate("button_Edit", add:GetLanguage()),function()
				local function GenerateThemeEditPanel(bg,saved)
					...
					for color_tab_name, tab_colors in pairs(addon_custom_skin.colors) do
						...
						local font_30 = esclib:AdaptiveFont("esclib", 30, 500)
						local font_24 = esclib:AdaptiveFont("esclib", 24, 500)
						...
						for color_name, color in pairs(tab_colors) do
							...
							function color_panel:Paint(w,h)
								...
								esclib.draw:ShadowText(""..color.r.." "..color.g.." "..color.b..(nalpha and (" "..color.a) or "").."", font_16, w-60, h*0.5, color, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER,1)

								if not nalpha then draw.RoundedBox(16,w-43,h*0.5-h*0.25,h*0.5,h*0.5, color) end
								draw.RoundedBox(16,w-45,h*0.5-h*0.25,h*0.5,h*0.5, color)
							end
							...
							function color_panel:DoClick()
								...
								change_panel:SetSize(self:GetWide(),content:GetTall()*0.4)
