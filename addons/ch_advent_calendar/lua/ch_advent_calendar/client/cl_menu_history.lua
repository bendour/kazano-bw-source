--[[
	Open the menu
--]]
net.Receive( "CH_Advent_Net_ShowHistory", function()
	...
	for i = 1, history_len do
		table.insert( history, {
			Nick = net.ReadString(),
			Square = net.ReadUInt( 5 ),
			Reward = net.ReadUInt( 8 ),
			Date = net.ReadUInt( 32 ),
		} )
	end
	
	CH_Advent.HistoryMenu( timeleft, day, history )
end )

function CH_Advent.HistoryMenu( timeleft, day, history )
	...
	GUI_AdventCalendarFrame.Paint = function( self, w, h )
		...
		-- Draw the top title.
		draw.SimpleText( CH_Advent.LangString( "Calendar History" ), "CH_Advent_Font_Outfit_Size25", w * 0.05, h * 0.13, CH_Advent.Colors.DarkRed, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
	...
	GUI_HistoryButton.Paint = function( self, w, h )
		draw.SimpleText( string.upper( CH_Advent.LangString( "History" ) ), "CH_Advent_Font_Outfit_Size7", w / 2, h * 0.1, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP )
	...
	GUI_TopInfoPanel.Paint = function( self, w, h )
		...
		draw.SimpleText( CH_Advent.LangString( "Opened" ), "CH_Advent_Font_Outfit_Size12", w * 0.02, h * 0.5, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
		
		draw.SimpleText( CH_Advent.LangString( "Square" ), "CH_Advent_Font_Outfit_Size12", w * 0.35, h * 0.5, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		
		draw.SimpleText( CH_Advent.LangString( "Name" ), "CH_Advent_Font_Outfit_Size12", w * 0.65, h * 0.5, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		
		draw.SimpleText( CH_Advent.LangString( "Reward" ), "CH_Advent_Font_Outfit_Size12", w * 0.98, h * 0.5, color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER )
	end
	...
	for k, v in SortedPairs( history, true ) do
		local square = v.Square
		local date = os.date( "%H:%M:%S - %d/%m/%Y" , v.Date )
		local name = v.Nick
		local reward = CH_Advent.Rewards[ v.Reward ] and CH_Advent.Rewards[ v.Reward ].Name or "Deleted Reward"
		...
		GUI_HistoryPanel:SetSize( CH_Advent.ScrW * ( #history > 10 and 0.7125 or 0.725 ), CH_Advent.ScrH * 0.04 )
		GUI_HistoryPanel.Paint = function( self, w, h )
			...
			draw.SimpleText( date, "CH_Advent_Font_Outfit_Size10", w * 0.02, h * 0.475, CH_Advent.Colors.BG, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
			
			draw.SimpleText( square, "CH_Advent_Font_Outfit_Size10", w * 0.35, h * 0.475, CH_Advent.Colors.BG, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			
			draw.SimpleText( name, "CH_Advent_Font_Outfit_Size10", w * 0.65, h * 0.475, CH_Advent.Colors.BG, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			
			draw.SimpleText( reward, "CH_Advent_Font_Outfit_Size10", w * 0.98, h * 0.475, CH_Advent.Colors.BG, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER )
		end
