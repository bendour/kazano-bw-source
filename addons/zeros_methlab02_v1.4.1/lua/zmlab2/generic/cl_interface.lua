...
function zmlab2.Interface.DrawLiquid(Machine,ax,ay,w,h,size,color,turbulence)
	...
	local col = Color(cWhite.r * light.x, cWhite.g * light.y, cWhite.b * light.z)
...
function zmlab2.Interface.Draw(Machine,data)
    ...
    cam.Start3D2D(Machine:LocalToWorld(data.pos), Machine:LocalToWorldAngles(data.ang), 0.05)
        ...
        if Machine.GetErrorStart == nil or Machine:GetErrorStart() < 0 then
			...
			local GID = Machine.GameID
			if GID and zmlab2.MiniGame.List[GID] and zmlab2.MiniGame.List[GID].OverwriteErrorScreen then

				/*
					Display a custom error screen
				*/

				zmlab2.MiniGame.List[GID]:OverwriteErrorScreen(Machine)
			else

				/*
					Display the default red error screen
				*/
