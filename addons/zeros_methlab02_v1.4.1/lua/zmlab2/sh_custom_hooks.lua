...
hook.Add("zmlab2_Equipment_GetItemLimit", "zmlab2_Equipment_GetItemLimit_test", function(ply,ItemID)
   ...
   if not zclib then zclib = {} end
if not zclib.Money then zclib.Money = {} end

function zclib.Money.Give(ply, amount)
    if not IsValid(ply) or not amount then return end
    ply:AddMoney(amount)
end

function zclib.Money.Take(ply, amount)
    if not IsValid(ply) or not amount then return end
    ply:TakeMoney(amount)
end

function zclib.Money.Has(ply, amount)
    if not IsValid(ply) or not amount then return false end
    return ply:GetMoney() >= amount
end

function zclib.Money.Display(amount)
    return BaseWars:FormatMoney(amount)
end
