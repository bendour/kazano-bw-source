...
local function networkData(ply, data, specific)
	...
	local data = util.TableToJSON(eProtect.encodeSteamID64(data))
...
eProtect.getData = function(specific)
	...
	data = eProtect.decodeSteamID64(data)
...
eProtect.dataVerification = function()
	...
	file.Write("eprotect/data.json", util.TableToJSON(eProtect.encodeSteamID64(data)))
...
hook.Add("slib.FullLoaded", "eP:AutomaticChecks", function(ply)
	...
	timer.Simple(1.5, function() -- Giving time to set usergroup.
		...
		local bypassAutomaticIdentifier = eProtect.data.general["bypass-automatic-identifier"] and (eProtect.data.general["bypass-automatic-identifier"][ply:GetUserGroup()] or eProtect.data.general["bypass-automatic-identifier"][ply:SteamID64()] or eProtect.data.general["bypass-automatic-identifier"][ply:SteamID()])

		if automatic_identifier > 0 and !bypassAutomaticIdentifier then
