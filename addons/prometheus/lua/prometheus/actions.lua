...
local function getRank(Ply)
	...
	if ULib || maestro || sam || sAdmin then
	...
	elseif xAdmin then
		if xAdmin.GetUserRank then -- V1
			CurrentRank = xAdmin.GetUserRank(Ply) or ""
		else -- V2
			CurrentRank = Ply:GetUserGroup() or ""
		end
	end
...
local function setRank(Ply, Rank)
	local Succ, Err

	if ULib then
		Succ, Err = pcall(ULib.ucl.addUser, Ply:SteamID(), nil, nil, Rank)
	elseif evolve then
		Succ, Err = pcall(Ply.EV_SetRank, Ply, Rank)
	elseif moderator then
		Succ, Err = pcall(moderator.SetGroup, Ply, Rank)
	elseif ASS_VERSION then
		Succ, Err = pcall(Ply.SetLevel, Ply, tonumber(Rank) )
		ASS_SaveRankings()
	elseif serverguard then
		Succ, Err = pcall(serverguard.player.SetRank, serverguard.player, Ply, Rank, false)
	elseif maestro then
		Succ, Err = pcall(Ply.SetUserGroup, Ply, Rank)
	elseif xAdmin then
		if xAdmin.SetUserRank then -- V1
			Succ, Err = pcall(xAdmin.SetUserRank, Ply, Rank)
		else -- V2
			Succ, Err = pcall(xAdmin.SetGroup, Ply, Rank)
		end
	elseif sam then
		Succ, Err = pcall(sam.player.set_rank, Ply, Rank)
	elseif sAdmin then
		Succ, Err = pcall(sAdmin.setRank, Ply, Rank, nil)
	else
		Succ = false
...
function Prometheus.SetActions()
	...
	Prometheus.AddAction({
		...
		runfunc = function(Ply, Values, TT)
			...
			if SetRank == CurrentRank then
				...
				local Succ, Err = setRank(Ply, SetRank)

				if !Succ then
					Err = string.Explode(":%d+:%s*", Err, true)[2] or Err
		...
		endfunc = function(Ply, Values)
			...
			if Values.rank_after == CurrentRank then
				...
				local Succ, Err = setRank(Ply, Values.rank_after)
		...
		runfunc = function(Ply, Values)
			if Ply.PS2_AddStandardPoints == nil then
				...
				if !Ply.PS2_Wallet then
					Ply.Prometheus.PS2SPoints = (Ply.Prometheus.PS2SPoints or 0) + (tonumber(Values.points) or 0)
				else
			...
			if Ply.PS2_AddPremiumPoints == nil then
				...
				if !Ply.PS2_Wallet then
					Ply.Prometheus.PS2PPoints = (Ply.Prometheus.PS2PPoints or 0) + (tonumber(Values.premium_points) or 0)
				else
	...
	Prometheus.AddAction({
		name = "pointshop2items",
		runfunc = function(Ply, Values)
			if Ply.PS2_EasyAddItem == nil then
				Prometheus.Error("Can't give " .. (Values.points or 0) .. " PS2 items to player: " .. Ply:Nick() .. "{" .. Ply:SteamID() .. "} because PS2 is not available.")
			else
				local ItemString = string.gsub(Values.items, "[^%w_,]", "")
				local ItemTable = string.Split(ItemString, ",")

				for _, j in ipairs(ItemTable) do
					Prometheus.Debug("Giving player: " .. Ply:Nick() .. "{" .. Ply:SteamID() .. "} " .. j .. " PS2 item.")
					if !Ply.PS2_Inventory then
						if !Ply.Prometheus.PS2Items then
							Ply.Prometheus.PS2Items = {}
						end
						table.Add(Ply.Prometheus.PS2Items, j)
					else
						Ply:PS2_EasyAddItem(j)
					end
				end
			end
		end
	...
	Prometheus.AddAction({
		...
		runfunc = function(Ply, Values)
			...
			local WepString = string.gsub(Values.string, "%s", "")
	...
	Prometheus.AddAction({
		name = "darkrpSalary",
		runfunc = function(Ply, Values)
			Ply.Prometheus.SalaryMultiplier = (Ply.Prometheus.SalaryMultiplier or 0) + tonumber(Values.multiplier)
			Prometheus.Debug("Added players: " .. Ply:Nick() .. "{" .. Ply:SteamID() .. "} salary multiplier. Now it is: x" .. Ply.Prometheus.SalaryMultiplier)
		end
	})
	...
	timer.Simple(0, function()
		hook.Run("PrometheusAddActions")
	end)
end


hook.Add("PS2_PlayerFullyLoaded", "PrometheusGivePS2Points", function(Ply)
	if Ply.Prometheus.PS2SPoints then
		Ply:PS2_AddStandardPoints(Ply.Prometheus.PS2SPoints)
		Ply.Prometheus.PS2SPoints = nil
	end

	if Ply.Prometheus.PS2PPoints then
		Ply:PS2_AddPremiumPoints(Ply.Prometheus.PS2PPoints)
		Ply.Prometheus.PS2PPoints = nil
	end

	if Ply.Prometheus.PS2Items then
		for _, j in ipairs(Ply.Prometheus.PS2Items) do
			Ply:PS2_EasyAddItem(j)
		end
		Ply.Prometheus.PS2Items = nil
	end
end)
