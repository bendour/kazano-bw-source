...
local tmysqlSucc = nil

if mysqlooSucc then
	include("prometheus/mysqloo.lua")
else
	tmysqlSucc = pcall(require, "tmysql4")
end
...
if !tmysqlSucc && !mysqlooSucc then
	...
	Prometheus.Error("Neither MySQLoo or tmysql4 module could be loaded! Please go to  http://wiki.prometheusipn.com/index.php?title=Installation:lua_prerequisites  and follow the instructions on how to install MySQLoo or tmysql4! Your server is " .. server .. " OS, so download the files for that OS!", true)
end
...
function Prometheus.DB.Error(Err, QueryS)
	...
	Prometheus.Error("Failed interacting with DB, FILE " .. File .. ", LINE " .. Debug.currentline .. ", QUERY \"" .. (QueryS or 'NO QUERY') .. "\" error: " .. (Err or "You have old version of tmysql4 or mysqloo, download latest, both files for the appropriate module! http://wiki.prometheusipn.com/index.php?title=Installation:lua_prerequisites") )
...
function Prometheus.DB.FetchPlayerBought(Callback, Ply)
	...
	Prometheus.DB.Query("SELECT a.active, a.expiretime, p.title FROM actions a INNER JOIN packages p ON a.package = p.id WHERE a.uid = '" .. Ply:SteamID64() .. "' AND (a.server LIKE '%\"" .. Prometheus.ServerID .. "\"%' OR a.server LIKE '%\"0\"%') GROUP BY a.active, a.expiretime, p.title, a.timestamp ORDER BY a.active, a.expiretime, p.title, a.timestamp DESC", LCallback)
end
