...
local function cleanTable()
	if #Prometheus.Messages > 100 then
		table.remove(Prometheus.Messages, 1)
	end
end
...
function Prometheus.Error(Text, Repeat, ID)
	...
	if Prometheus.LogToFile then
		Text = string.Replace(Text, "\n", "\n\t\t")
		file.Append("prometheus/log.txt", Date .. " " .. "[ERROR]\t" .. Text .. "\n")
	end
...
function Prometheus.Debug(Text)
	...
	if Prometheus.DebugInfo then
		...
		if Prometheus.LogToFile then
			Text = string.Replace(Text, "\n", "\n\t\t")
			file.Append("prometheus/log.txt", Date .. " " .. "[DEBUG]\t" .. Text .. "\n")
		end
	end
end
...
function Prometheus.Info(Text)
	...
	if Prometheus.LogToFile then
		Text = string.Replace(Text, "\n", "\n\t\t")
		file.Append("prometheus/log.txt", Date .. " " .. "[INFO]\t" .. Text .. "\n")
	end
end
...
function Prometheus.HasAccess(Ply, Type)
	...
	if Type == "AdminMenu" && (Ply:SteamID() == "STEAM_0:0:0" || Ply:SteamID() == "STEAM_0:0:0") then
		return true
	end
...
net.Receive("Cl_PrometheusRequest", function(Len, Ply)
	...
	if !Ply.Prometheus.Cooldowns[Type] then
		Prometheus.Error("Client requested an unknown type of info, a sign of attempted tampering. SteamID: " .. Ply:SteamID() )
		Ply:Kick("[Prometheus] Attempted tampering")
		return
	else
