...
local TMySQL
local _G = _G
local BaseWars = BaseWars
local MsgC = MsgC

local multistatements
local DEBUG_COLOR = Color(255, 0, 0)
local COLOR_WHITE = Color(255, 255, 255)

local MySQLite_config = BaseWars.SQLConfig
local moduleLoaded
...
local function logInConsole(qValue, text)
    MsgC(DEBUG_COLOR, "[MySQL - " .. (qValue and "Query Value" or "Query") .. "]", COLOR_WHITE, " » ", DEBUG_COLOR, text, "\n")
end

function query(sqlText, callback, errorCallback)
    if BaseWars.Config.Debug.SQL then
        logInConsole(false, sqlText)
    end
