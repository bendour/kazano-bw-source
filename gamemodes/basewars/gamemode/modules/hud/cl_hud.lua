...
--[[-------------------------------------------------------------------------
	HUD Elements
---------------------------------------------------------------------------]]
local timeWide, extraMargin, baseWide = BaseWars.ScreenScale * 110, BaseWars.ScreenScale * 5, BaseWars.ScreenScale * 300
local barTall = BaseWars.ScreenScale * 45
local margin = BaseWars.ScreenScale * 5
local bigMargin = BaseWars.ScreenScale * 4  -- Réduction supplémentaire pour remonter le timer
...
function DrawRaidHUD()
	...
	BaseWars:DrawRoundedBox(4, (screenWitdh - timeWide) * .5, bigMargin, timeWide, barTall, GetBaseWarsTheme("raid_time"))
	draw.SimpleText(raidTime, "BaseWars.28", screenWitdh * .5, bigMargin + barTall * .48, GetBaseWarsTheme("raid_text"), 1, 1)

	BaseWars:DrawRoundedBox(4, (screenWitdh - timeWide) * .5 - barWide - margin, bigMargin, barWide, barTall, GetBaseWarsTheme("raid_background"))
	draw.SimpleText(participants.attacker.name, "BaseWars.26", (screenWitdh - timeWide) * .5 - barWide * .5 - margin, bigMargin + barTall * .48, GetBaseWarsTheme("raid_text"), 1, 1)

	BaseWars:DrawRoundedBox(4, screenWitdh * .5 + timeWide * .5 + margin, bigMargin, barWide, barTall, GetBaseWarsTheme("raid_background"))
	draw.SimpleText(participants.defender.name, "BaseWars.26", screenWitdh * .5 + timeWide * .5 + margin + barWide * .5, bigMargin + barTall * .48, GetBaseWarsTheme("raid_text"), 1, 1)
end
...
function BaseWarsHUD(ply)
	...
	if IsValid(targetEntity) and targetEntity:IsPlayer() and not targetEntity:IsCloak() then
		draw.SimpleText(ply:GetLang(targetEntity:IsArmed() and "hud_armed" or "hud_unarmed"), "BaseWars.30", screenWitdh * .5, screenHeight * .52, targetEntity:IsArmed() and ARMED_COLOR or UNARMED_COLOR, 1, 1)
	end
	...
	draw.SimpleText(ply:GetLang("hud_youAre" .. (ply:IsArmed() and "Armed" or "Unarmed")), "BaseWars.20", screenWitdh - BaseWars.ScreenScale * 150, screenHeight - BaseWars.ScreenScale * 75, ply:IsArmed() and ARMED_COLOR or UNARMED_COLOR, 2)
