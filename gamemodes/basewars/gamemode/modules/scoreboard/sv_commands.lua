BaseWars:AddChatCommand("god", function(ply, args)
	...
	if IsValid(target) then
		...
		BaseWars:Notify(target, target:IsGodmode() and "#command_god" or "#command_ungod", NOTIFICATION_GENERIC, 5)
	else
...
BaseWars:AddChatCommand("cloak", function(ply, args)
	...
	if IsValid(target) then
		...
		BaseWars:Notify(target, target:IsCloak() and "#command_cloak" or "#command_uncloak", NOTIFICATION_GENERIC, 5)
	else
