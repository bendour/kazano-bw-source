...
function GM:ScoreboardShow()
	-- Désactivé - ne fait plus rien quand on appuie sur Tab
	-- Les fonctionnalités sont conservées mais l'interface n'apparaît plus
...
local icons = {
	...
	premium = Material("materials/basewars_materials/scoreboard/premium.png", "smooth"),
	admin = Material("materials/basewars_materials/scoreboard/admin.png", "smooth"),
    superadmin = Material("materials/basewars_materials/scoreboard/admin.png", "smooth"),
}
...
function PANEL:AddPlayer(ply)
	...
	playerPanel.Paint = function(s,w,h)
		...
		-- Vérifie si le joueur est SuperAdmin via BaseWars
		if BaseWars:IsSuperAdmin(ply) then
			userGroup = "admin"
			
		-- Vérifie si le joueur est VIP
		elseif BaseWars:IsAdmin(ply) then
			userGroup = "admin"		
		...
		-- Vérifie si le joueur est Premium
		elseif ply:GetUserGroup() == "Premium" then
			userGroup = "premium"
			
		-- Si aucun autre groupe ne correspond, le joueur reste "user"
		end
...
function PANEL:PlayerPanel(ply)
	...
	self.PlayerInfos.Avatar = self.PlayerInfos:Add("BaseWars.Avatar")
	...
	self.PlayerInfos.Avatar:SetPlayer(ply, 256)
