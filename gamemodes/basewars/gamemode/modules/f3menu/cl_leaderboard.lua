...
local thisPanel
local leftDisplay, rightDisplay = "time_played", "prestige"
...
function PANEL:Init()
    ...
    self.rowWide = (self.w - bigMargin * 2 - margin * 2 - rowTall * 2) * .5
    self.colors = {
        text = GetBaseWarsTheme("bws_text") or color_white,
        darkText = GetBaseWarsTheme("bws_darkText") or Color(190,190,190),
        ...
        contentBackground = GetBaseWarsTheme("bws_contentBackground") or Color(16,16,18)
    }
    ...
    self.LeftPanel:SetWide((self.w - bigMargin * 2) * .5)
    ...
    self.LeftPanel.Scroll:GetVBar():SetWide(bigMargin)
    self.LeftPanel.Scroll:PaintScrollBar("bws")
    ...
    self.RightPanel:SetWide((self.w - bigMargin * 2) * .5)
    ...
    self.RightPanel.Scroll:GetVBar():SetWide(bigMargin)
    self.RightPanel.Scroll:PaintScrollBar("bws")
...
function PANEL:BuildLeft(what)
    ...
    if not self.LeftPanel.Topbar then
        ...
        self.LeftPanel.Topbar.__hoverProg = 0
        self.LeftPanel.Topbar.Paint = function(s,w,h)
            ...
            BaseWars:DrawRoundedBox(roundness, 0, 0, w, elementTall, ColorAlpha(self.colors.contentBackground, 90))
            local a = math.floor(s.__hoverProg * 30)
            if a > 0 then
                draw.RoundedBox(roundness, 0, 0, w, elementTall, Color(255,255,255,a))
                local accent = self.colors.accent
                surface.SetDrawColor(accent.r or 255, accent.g or 60, accent.b or 70, 90)
                surface.DrawOutlinedRect(1, 1, w - 2, elementTall - 2)
            end
    ...
    for k, v in ipairs(leaderboardData[what]) do
        ...
        row.Paint = function(s,w,h)
            BaseWars:DrawRoundedBox(roundness, 0, 0, h, h, ColorAlpha(self.colors.contentBackground, 90))
            draw.SimpleText("#" .. k, "BaseWars.18", h * .5, h * .5, color, 1, 1)

            BaseWars:DrawRoundedBox(roundness, h + margin, 0, self.rowWide, h, ColorAlpha(self.colors.contentBackground, 90))
...
function PANEL:BuildRight(what)
    ...
    if not self.RightPanel.Topbar then
        ...
        self.RightPanel.Topbar.__hoverProg = 0
