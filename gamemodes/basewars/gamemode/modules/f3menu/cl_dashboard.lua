...
function PANEL:Init()
    ...
    self.colors = {
        ...
        text = GetBaseWarsTheme("bws_text") or color_white,
        darkText = GetBaseWarsTheme("bws_darkText") or Color(120,120,120),
        contentBackground = GetBaseWarsTheme("bws_contentBackground") or Color(22,20,22),
        contentBackground2 = GetBaseWarsTheme("bws_contentBackground2") or Color(18,16,18)
    }
    ...
    self.Left.StaffOnline.Title.Paint = function(s,w,h)
        BaseWars:DrawRoundedBox(roundness, 0, 0, w, h, ColorAlpha(self.colors.contentBackground, 90))
    ...
    self.Left.StaffOnline.Scroll:GetVBar():SetWide(bigMargin)
    self.Left.StaffOnline.Scroll:PaintScrollBar("bws")
    ...
    self.Left.Bounties.Scroll:GetVBar():SetWide(bigMargin)
    self.Left.Bounties.Scroll:PaintScrollBar("bws")
    ...
    self.Right.Scroll:GetVBar():SetWide(bigMargin)
    self.Right.Scroll:PaintScrollBar("bws")
    ...
    for k, v in SortedPairs(BaseWars:GetChatCommands()) do
        ...
        command.__hoverProg = 0
        command.Paint = function(s,w,h)
            local bgCol = self.colors[access and "contentBackground" or "contentBackground2"]
            BaseWars:DrawRoundedBox(roundness, 0, 0, w, h, ColorAlpha(bgCol, 90))
            ...
            if a > 0 then
                draw.RoundedBox(roundness, 0, 0, w, h, Color(255,255,255,a))
            end

            local borderColor = self.colors.accent
        ...
        if hasArgs and access then
            ...
            command.Copy:SetColor(self.colors.accent, true)
            command.Copy.LerpFunc = function(s)
                return s:IsHovered()
            end
            command.Copy.Draw = function(s,w,h)
                BaseWars:DrawMaterial(icons["copy"].icon, w - h * .5, h * .5, icons["copy"].size, icons["copy"].size, color_white, 0)
            end
        ...
        if not hasArgs and access then
            command.ExecuteCommand = command:Add("BaseWars.Button")
            ...
            command.ExecuteCommand:SetColor(self.colors.accent, true)
            command.ExecuteCommand.LerpFunc = function(s)
                return s:IsHovered()
            end
            command.ExecuteCommand.Draw = function(s,w,h)
                BaseWars:DrawMaterial(icons["execute_command"].icon, w - h * .5, h * .5, icons["execute_command"].size, icons["execute_command"].size, color_white, 0)
            end
