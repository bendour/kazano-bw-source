...
function PLAYER:QuitFaction(name)
	if not name then
		return
	end
...
function PLAYER:ChangeFactionPassword(name, password)
	if not name or not password then
		return
	end
...
function PLAYER:KickPlayerFromFaction(name, ply)
	if not name or not ply or not IsValid(ply) then
		return
	end
...
function PLAYER:TransferFactionLeadership(name, ply)
	if not name or not ply or not IsValid(ply) then
		return
	end
...
function PLAYER:ChangeFactionFriendlyFire(name, bool)
	if not name or bool == nil then
		return
	end
...
net.Receive("BaseWars:Factions:Player:QuiFaction", function(len, ply)
	...
	ply:QuitFaction(ply:GetFaction())
end)
...
net.Receive("BaseWars:Factions:Player:ChangePassword", function(len, ply)
	...
	ply:ChangeFactionPassword(name, password)
end)
...
net.Receive("BaseWars:Factions:Player:KickPlayer", function(len, ply)
	...
	ply:KickPlayerFromFaction(name, target)
end)
...
net.Receive("BaseWars:Factions:Player:TransferLeadership", function(len, ply)
	...
	ply:TransferFactionLeadership(name, target)
end)
...
net.Receive("BaseWars:Factions:Player:ToggleFriendlyFire", function(len, ply)
	...
	ply:ChangeFactionFriendlyFire(name, bool)
end)
