...
BaseWars:AddFactionInteraction("faction_createFaction", buttonSize * 6 + bigMargin * 3 + margin * 3, function(parent)
    ...
    parent.Button:SetAccentColor(GetBaseWarsTheme("gen_accent"))
    parent.Button.LerpFunc = function(s)
        return s:IsHovered()
    end
	...
	parent.Button.DoClick = function(s)
        ...
        local function validateAndCreate(name, password, color, btn)
            if localPlayer:InRaid() then
                BaseWars:Notify("#faction_inRaid", NOTIFICATION_ERROR, 5)
                btn:Disable(1.5, theme.disabled, btn.Draw)
                return
            end
            ...
            btn:CustomTempDraw(1.5, theme.green, btn.Draw)
            localPlayer:CreateFaction(name, password, color)
        end

        local name = string.Trim(parent.Name:GetText() or "")
        local password = string.Trim(parent.Password:GetText() or "")
        local color = parent.ColorPicker and parent.ColorPicker.GetColor and parent.ColorPicker:GetColor() or Color(255, 255, 255)
        validateAndCreate(name, password, color, s)
    end
end)
...
BaseWars:AddFactionInteraction("faction_joinFaction", buttonSize * 3 + bigMargin * 2 + margin * 2, function(parent)
	...
	parent.Button:SetTall(buttonSize)
    parent.Button:SetColor(theme.contentBackground, true)
    parent.Button:SetAccentColor(GetBaseWarsTheme("bws_accent") or GetBaseWarsTheme("gen_accent"))
    parent.Button.LerpFunc = function(s)
        return s:IsHovered()
    end
	parent.Button.Draw = function(s,w,h)
...
BaseWars:AddFactionInteraction("faction_frendlyFire", buttonSize + bigMargin * 2, function(parent)
    ...
    parent.BackPanel.Paint = function(s,w,h)
        BaseWars:DrawRoundedBox(12, 0, 0, w, h, ColorAlpha(theme.contentBackground, 90))
        local accent = GetBaseWarsTheme("bws_accent") or GetBaseWarsTheme("gen_accent") or color_white
        surface.SetDrawColor(accent.r or 255, accent.g or 255, accent.b or 255, 30)
        surface.DrawOutlinedRect(1, 1, w - 2, h - 2)

        draw.SimpleText(localPlayer:GetLang("faction_frendlyFire"), "BaseWars.18", bigMargin, h * .5, theme.text, 0, 1)
    end
    ...
    parent.BackPanel.Toggle:SetBackgroundColor(theme.contentBackground)
