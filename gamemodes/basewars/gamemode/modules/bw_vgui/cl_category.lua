...
function PANEL:Init()
	...
	self.Top:Dock(TOP)
	self.Top:SetTall(BaseWars.ScreenScale * 36)
	self.Top:SetText("")
	self.Top.Paint = function(s, w, h)
		-- Header avec séparation claire: fond translucide, barre accent à gauche et underline pleine largeur
		local titleCol = self:GetColor()
		local textCol = self:GetTextColor()
		local accent = GetBaseWarsTheme("bws_accent") or self:GetAccentColor() or textCol

		BaseWars:DrawRoundedBoxEx(4, 0, 0, w, h, ColorAlpha(titleCol, 45), true, true, false, false)

		-- Barre accent verticale pour identification rapide
		surface.SetDrawColor(accent.r or 200, accent.g or 60, accent.b or 70, 80)
		surface.DrawRect(0, 0, math.max(2, math.floor(BaseWars.ScreenScale * 1.5)), h)

		draw.SimpleText(self:GetName(), "BaseWars.24", margin + bigMargin, h * .5, textCol, 0, 1)

		-- Underline pleine largeur pour une séparation nette
		local underlineH = 2
		local contentPad = margin + bigMargin
		local underlineW = math.max(0, w - contentPad * 2)
		surface.SetDrawColor(accent.r or 200, accent.g or 60, accent.b or 70, 55)
		surface.DrawRect(contentPad, h - underlineH - 4, underlineW, underlineH)
	end
