...
function GM:EntityTakeDamage(ent, dmg)
	...
	if ent:IsPlayer() and att:IsPlayer() then
		...
		if not BaseWars.Config.ShootPeopleInRaid and (ent:InRaid() and not att:InRaid()) then
			BaseWars:Notify(att, "#raid_cantShootPersonInRaid", NOTIFICATION_WARNING, 8)
			return true
		end
	...
	if att:IsPlayer() and not ent:IsPlayer() then
		-- Exception pour les entités PumpkinNight (Boss, Minions, Ghost, etc.)
		local entClass = ent:GetClass()
		if entClass == "zpn_boss" or entClass == "zpn_minion" or entClass == "zpn_ghost" or 
		   entClass == "zpn_destructable" or entClass == "zpn_bomb" or entClass == "zpn_fireball" then
			return false -- Permet les dégâts sur ces entités
		end
		...
		if ent:IsClass("prop_physics") then
			if BaseWars:RaidGoingOn() and entityOwner:Enemy(att) then
				-- Les blowtorchs gèrent leurs propres dégâts, on bloque ici
				local wep = att:GetActiveWeapon()
				if IsValid(wep) then
					local wepClass = wep:GetClass()
					-- Si c'est une blowtorch, ignorer (elle gère ses propres dégâts)
					if wepClass == "bw_blowtorch" or wepClass == "bw_blowtorch_vip" or wepClass == "bw_blowtorch_premium" then
						return true -- Bloquer les dégâts ici, la blowtorch les applique directement
					end
				end
				
				-- Nerf des armes normales: seulement 1 PV par hit sur les props
...
function GM:ShutDown()
	if BaseWars.Config.ShutdownRefund then
		BaseWars:RefundAll()
		BaseWars:Log("Refunded everyone!")
	end
end
...
function PLAYER:SetArmed(bool)
	self:SetNWBool("BaseWars.Armed", bool or false)
end
