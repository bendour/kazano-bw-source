...
function GM:PlayerInitialSpawn(ply)
	...
	-- S'assurer que basewarsShopCooldowns est correctement initialisé
	if not ply.basewarsShopCooldowns then
	...
	--BaseWars:ServerStatus("{" .. ply:Name() .. "} est apparu. (" .. player.GetCount() .. "/" .. game.MaxPlayers() .. ")")
end
...
function GM:PlayerDisconnected(ply)
	if BaseWars:RaidGoingOn() then
		local raidType = BaseWars:GetRaidType()

		if raidType == RAIDTYPE_SOLO then
			if ply:InRaid() then
				BaseWars:CancelRaid(ply, "raid_playerDisconnected", ply:Name())
			end
		elseif raidType == RAIDTYPE_FACTION then
			if ply:IsFactionLeader() then
				BaseWars:CancelRaid(ply, "raid_playerDisconnected", ply:Name())
			end
		end
	end
...
--[[-------------------------------------------------------------------------
	MARK: GM:PlayerDeath()
---------------------------------------------------------------------------]]
local pew_pew = Sound("bw_pew_pew.wav")
function GM:PlayerDeath(ply, inflictor, attacker)
	ply.DeathTime = CurTime()
	ply:EmitSound(pew_pew, 140, 100, .8)
...
hook.Add("player_disconnect", "BaseWars:player_disconnect", function(data)
	...
	--BaseWars:ServerStatus("{" .. data.name .. "} à quitté le serveur. (" .. count .. "/" .. game.MaxPlayers() .. ")")
end)

--[[gameevent.Listen("player_connect")
hook.Add("player_connect", "BaseWars:player_connect", function(data)
	BaseWars:ServerStatus("{" .. data.name .. "} à rejoin le serveur. (" .. player.GetCount() .. "/" .. game.MaxPlayers() .. ")")
end)]]--
...
net.Receive("BaseWars:OpenSpawnpointMenu", function(len, ply)
	-- SECURITY: Player must be dead to respawn
	if ply:Alive() then return end
	
	-- SECURITY: Rate limiting
	if ply._lastSpawnAttempt and (CurTime() - ply._lastSpawnAttempt) < 1.0 then return end
	ply._lastSpawnAttempt = CurTime()

	local spawnpoint = net.ReadEntity()

	-- SECURITY: Only allow spawning at valid spawnpoints owned by the player
	if IsValid(spawnpoint) then
		if spawnpoint:IsClass("bw_spawnpoint") and spawnpoint:CPPIGetOwner() == ply then
			ply:Spawn()
			ply:SetPos(spawnpoint:GetPos() + Vector(0, 0, 5))
			ply:SetEyeAngles(Angle(0, 90, 0))
		end
	else
		-- Allow world spawn for dead players with no spawnpoints
		ply:Spawn()
		ply:SetPos(Vector(math.Rand(-3398, -3008), math.Rand(-3659, -2979), 1813))
