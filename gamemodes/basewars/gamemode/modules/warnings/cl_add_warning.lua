...
function PANEL:Init()
    ...
    self.Frame.Title.Paint = function(s,w,h)
        ...
        draw.SimpleText(Format(self.localPlayer:GetLang("warnings_admin_addWarningTitle"), self.playerName), "BaseWars.24", w * .5, h * .5, self.colors.text, 1, 1)
    end
    ...
    self.Frame.Middle.Entry.OnEnter = function(s)
        ...
        net.Start("BaseWars:Warnings:AddWarn")
            net.WriteString(self.player_id64)
    ...
    self.Frame.Bottom.Confirm.Draw = function(s,w,h)
        draw.SimpleText(self.localPlayer:GetLang("warnings_admin_addWarningButton"), "BaseWars.18", w * .5, h * .5, self.colors.text, 1, 1)
    end
...
function PANEL:SetPlayerID(player_id64)
    self.player_id64 = player_id64
end
...
vgui.Register("BaseWars.Warnings.AddWarning", PANEL, "EditablePanel")