...
function PANEL:Init()
    ...
    self.TopBar.Paint = function(s,w,h)
        ...
        draw.SimpleText(Format(self.localPlayer:GetLang("adminmenu_globalImmunity"), time > 0 and BaseWars:FormatTime2(time) or self.localPlayer:GetLang("none")), "BaseWars.20", h * .25, h * .5, self.colors.text, 0, 1)
    end
    ...
    self.TopBar.Reset.Draw = function(s,w,h)
        draw.SimpleText(self.localPlayer:GetLang("adminmenu_reset"), "BaseWars.18", w * .5, h * .5, self.colors.text, 1, 1)
    end
