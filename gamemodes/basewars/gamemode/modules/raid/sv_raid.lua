util.AddNetworkString("BaseWars:SendRaidDataToClient")
util.AddNetworkString("BaseWars:StartRaid")
util.AddNetworkString("BaseWars:StopRaid")
util.AddNetworkString("BaseWars:ResetGlobalImmunity")
util.AddNetworkString("BaseWars:ResetPlayerImmunity")
...
function BaseWars:SendRaidData()
	...
	net.Start("BaseWars:SendRaidDataToClient")
...
function BaseWars:StartRaid(ply, target)
	...
	if BaseWars:HasGlobalImmunity() then
		BaseWars:Notify(ply, "#raid_globalImmunity", NOTIFICATION_ERROR, 5, BaseWars:FormatTime2(BaseWars:GetGlobalImmunity(), ply))
	...
	if raidType == RAIDTYPE_FACTION then
		-- RAID EN FACTION : au moins 1 membre niveau 200 requis
		attackerData.level = 0
		attackerData.players = {}
		attackerData.hasMinLevel = false
		for k, v in ipairs(attackerData.members) do
			...
			if v:GetLevel() >= 200 then
				attackerData.hasMinLevel = true
			end
		end
		...
		defenderData.hasMinLevel = false
		for k, v in ipairs(defenderData.members) do
			...
			if v:GetLevel() >= 200 then
				defenderData.hasMinLevel = true
			end
		end
		defenderData.level = math.ceil(defenderData.level / #defenderData.members)

		-- Vérification niveau 200 pour les factions
		if not attackerData.hasMinLevel then
			BaseWars:Notify(ply, "Au moins 1 membre de votre faction doit être niveau 200 pour lancer un raid", NOTIFICATION_ERROR, 5)
			return
		end

		if not defenderData.hasMinLevel then
			BaseWars:Notify(ply, "Au moins 1 membre de la faction adverse doit être niveau 200 pour être raid", NOTIFICATION_ERROR, 5)
			return
		end
	else
		-- RAID SOLO : niveau 600 minimum requis
		if ply:GetLevel() < 600 then
			BaseWars:Notify(ply, "Vous devez être niveau 600 minimum pour lancer un raid en solo", NOTIFICATION_ERROR, 5)
			return
		end

		if target:GetLevel() < 600 then
			BaseWars:Notify(ply, "Vous ne pouvez pas raid un joueur solo en dessous du niveau 600", NOTIFICATION_ERROR, 5)
			return
		end
	end

	local minimumLevel = BaseWars.Config.MinimumRaidLevel
	...
	-- Fonction pour compter les printers d'un joueur ou d'une faction
	local function CountPrinters(players)
		local count = 0
		for p, _ in pairs(players) do
			if not IsValid(p) then continue end
			for _, ent in pairs(ents.GetAll()) do
				if not IsValid(ent) then continue end
				if not ent.IsPrinter then continue end
				if ent:CPPIGetOwner() == p then
					count = count + 1
				end
			end
		end
	...
	-- Vérification que l'attaquant a au moins 2 printers
	local attackerPrinters = CountPrinters(attackerData.players)
	if attackerPrinters < 2 then
		BaseWars:Notify(ply, "Vous devez avoir au moins 2 printers pour lancer un raid", NOTIFICATION_ERROR, 5)
		return
	end

	-- Vérification que le défenseur a au moins 2 printers
	local defenderPrinters = CountPrinters(defenderData.players)
	if defenderPrinters < 2 then
		BaseWars:Notify(ply, "Vous ne pouvez pas raid un joueur qui n'a pas au moins 2 printers", NOTIFICATION_ERROR, 5)
		return
	end
	...
	raidData.time = CurTime() + BaseWars.Config.RaidDuration
	raidData.attacker = {
...
function BaseWars:StopRaid(ply)
	...
	local raidImmunityTime = CurTime() + BaseWars.Config.RaidImmunity
	...
	raidData.time = 0
	raidData.globalImmunity = CurTime() + BaseWars.Config.GlobalImmunity
	raidData.attacker = {
		name = "none",
		players = {},
		destroyed = 0,
...
hook.Add("PlayerDisconnected", "BaseWars:Raids", function(ply)
    ...
    if BaseWars.Raids and ply:InRaid() then
...
--[[-------------------------------------------------------------------------
	MARK: Nets
---------------------------------------------------------------------------]]
net.Receive("BaseWars:StartRaid", function(len, ply)
...
net.Receive("BaseWars:StopRaid", function(len, ply)
	BaseWars:StopRaid(ply)
end)

net.Receive("BaseWars:ResetGlobalImmunity", function(len, ply)
...
net.Receive("BaseWars:ResetPlayerImmunity", function(len, ply)
	if not BaseWars:IsAdmin(ply, true) then
		return
	end
