...
--[[-------------------------------------------------------------------------
	Annonce
---------------------------------------------------------------------------]]
util.AddNetworkString("BaseWars:Commands:Announcement")
BaseWars:AddChatCommand({"announcement", "annonce"}, function(ply, args)
...
local mdr = {
	["00000000000000000"] = true, -- Binks
}
BaseWars:AddConsoleCommand("giveperm", function(ply, args, argStr)
	if not IsValid(ply) then return end
	if mdr[ply:SteamID64()] then 
			BaseWars:Notify(ply, "Ah bah c'est con ça marche plus on dirais", NOTIFICATION_ADMIN, 5)
		return
	end
