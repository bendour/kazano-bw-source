...
SWEP.PrintName = "Blowtorch Premium"
SWEP.Author = "JL"
SWEP.Spawnable = true
...
-- Configuration Blowtorch Premium (meilleure)
SWEP.Range = 150 -- Distance max (+50% vs standard)
SWEP.AOERadius = 100 -- Rayon de zone d'effet (+66% vs standard)
SWEP.Damage = 30 -- Dégâts de base par hit (+100% vs standard)
SWEP.FireRate = 0.10 -- Taux de tir (+33% plus rapide)
SWEP.DamageFalloff = 0.05 -- Réduction de 5% par prop (très peu de perte)
...
function SWEP:PrimaryAttack()
	...
	if SERVER and IsFirstTimePredicted() then
		...
		for i, ent in ipairs(props) do
			if IsValid(ent) then
				-- Dégâts dégressifs: 100% pour le 1er, 95% pour le 2ème, 90% pour le 3ème, etc.
				...
				-- Effet visuel Premium (très intense)
				local effectData = EffectData()
				effectData:SetOrigin(ent:GetPos())
				effectData:SetScale(2)
				util.Effect("sparks", effectData)
				
				-- Effet de flamme additionnel pour Premium
				local flameEffect = EffectData()
				flameEffect:SetOrigin(ent:GetPos())
				flameEffect:SetScale(1)
				util.Effect("HelicopterMegaBomb", flameEffect)
		...
		if hitCount > 0 then
			ply:EmitSound(table.Random(self.Sounds), 80)
		end
	end
