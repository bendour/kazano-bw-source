...
function ENT:Draw()
    ...
    cam.Start3D2D(pos, angle, .1)
        ...
        if hasBank then
            ...
            local capacity = self:GetCapacity() or 1
            local time = (capacity - money) / math.max((printAmount / interval), 0.001)
            ...
            -- Printer Infos
            local moneyPerSec = interval > 0 and (printAmount / interval) or 0
            BaseWars:DrawRoundedBox(2, 10, 86, w - 20, 15, self.SubColor)
            BaseWars:DrawRoundedBox(2, 10, 86, (w - 20) * self.moneyBar, 15, textColor)
            draw.SimpleText((BaseWars:FormatMoney(money, true) or "0") .. " / " .. (BaseWars:FormatMoney(capacity, true) or "0") .. "(+ " .. (BaseWars:FormatMoney(moneyPerSec, true) or "0") .. ")", "BaseWars.Printer.Info", w * .5, 101, textColor, 1)
            ...
            draw.SimpleText(ply:GetLang("printer_xp"):format(BaseWars:FormatNumber(BaseWars:CalculatePlayerXP(self:CPPIGetOwner(), BaseWars:CalculateXPFromMultiplier(money) * BaseWars.Config.XPMult) or 0)), "BaseWars.Printer.Info", w * .5, h - 98, textColor, 1, TEXT_ALIGN_BOTTOM)
