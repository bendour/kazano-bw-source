...
function ENT:UpgradeHealth(ply)
	...
	self:SetMaxHealth(self.PresetHealth * (self:GetHealthLevel() + 1) ^ 1.2)
...
function ENT:ThinkFunc()
	...
	if IsValid(owner) and owner:GetPrestigePerk("bankInterest") > 0 and CurTime() >= self.InterestTime then
		...
		if bankMoney > 0 then
			local interestAmount = bankMoney * .1
			owner:AddMoney(interestAmount)
			
			-- Donner de l'XP en plus de l'argent (1 XP pour chaque 10$ d'intérêt)
			local xpAmount = math.floor(interestAmount / 10)
			if xpAmount > 0 then
				owner:AddXP(xpAmount)
			end
			
			BaseWars:Notify(owner, "#prestige_bankInterest", NOTIFICATION_GENERIC, 5, BaseWars:FormatMoney(interestAmount))
		end
